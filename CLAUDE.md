...
| `server/internal/handler/` | Handler 层 — 全部 10 个模块已实现：auth/user/role/chat/ticket/knowledge/dashboard/config/audit/message |
| `server/internal/service/` | Service 层 — 全部 10 个模块已实现：auth/user/role/chat/ticket/knowledge/dashboard/config/message/scheduler |
| `server/internal/repository/` | Repository 层 — 全部 8 个模块已实现：user/role/config/ticket/knowledge/chat/audit/message |
| `server/internal/model/` | GORM 数据模型 — 全部 10 个模型文件（对应 TECH.md §4.2 表结构） |
| `server/internal/adapter/` | 外部服务适配层（RagClient / StorageClient — 已实现） |
